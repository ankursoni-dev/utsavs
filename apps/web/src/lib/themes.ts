 ...
 *
 * `theme` is stored on the Event entity as an enum string. The ThemeProvider
 * looks up the token set by name and injects CSS custom properties on the
 * subtree, so every guest-facing component reads `var(--theme-primary)` etc.
 *
